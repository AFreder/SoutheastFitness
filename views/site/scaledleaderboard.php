...
<p>
	<?php
		if($message[0]->message != '')
		{
			...
			echo "<tr>"; 
			echo "<td style='text-align:center'><strong>Event Update: </strong>" . $message[0]->message . "</td>";
			echo "</tr>";
			echo "</table>";
			echo "</div>";
			echo "</center>";
	...
	<table class="table table-striped" style="border:1px solid #999999">
		...
		function eventFinalized ($array)
		{
			$flag = 'Y';
			$i=0;
			foreach ($array as $tmpArr) 
			{
				if($tmpArr[1] == 0)
				{
					$flag = 'N';
		...
		if(count($compName) > 0)
		{
			...
			{
				$event1flag = eventFinalized($event1byID);
				$event2flag = eventFinalized($event2byID);
				$event3flag = eventFinalized($event3byID);
				$event4flag = eventFinalized($event4byID);
				if($event2flag == 'Y' && $event1flag == 'Y'){ $event2FinalScore = $event2byID[$i][1]; } else { $event2FinalScore = 0; }
				if($event3flag == 'Y' && $event2flag == 'Y' && $event1flag == 'Y'){ $event3FinalScore = $event3byID[$i][1]; } else { $event3FinalScore = 0; }
				if($event4flag == 'Y' && $event3flag == 'Y' && $event2flag == 'Y' && $event1flag == 'Y'){ $event4FinalScore = $event4byID[$i][1]; } else { $event4FinalScore = 0; }
				
				$compFinal[$i][0] = $event1byID[$i][1] + $event2FinalScore + $event3FinalScore + $event4FinalScore;
				if($compFinal[$i][0] == 0)
				{
